...
output_image_path = r"P:\petes_code\github\LunarPi\Training\detected_output_tweaked.jpg"
tile_sizes = [(684, 332), (332, 332)]  # Large for rover/horizon, small for rocks
strides = [342, 83]  # Tighter stride for rocks
confidence_thresholds = [0.5, 0.4]  # Lowered for rocks
...
def merge_boxes(detections, iou_threshold=0.5):  # Stricter merge for rocks
    merged = []
...
def process_image(image_path):
    ...
    # Post-process to force empty areas as empty_terrain
    h_threshold = h * 0.4  # Increased to 40% for horizon
    ...
    for y in range(0, h - large_tile_h + 1, large_stride):
        for x in range(0, w - large_tile_w + 1, large_stride):
            ...
            if not overlaps:
                if y < h_threshold and not any(d[0] == "horizon" for d in all_detections if d[2][1] <= y < d[2][3]):
                    all_detections.append(("horizon", 0.6, (x, y, x + large_tile_w, y + large_tile_h)))
                elif y >= h_threshold and not any(d[0] == "empty_terrain" for d in all_detections if d[2][1] <= y < d[2][3]):
                    all_detections.append(("empty_terrain", 0.6, (x, y, x + large_tile_w, y + large_tile_h)))
